"""Purina vet-channel scraper (purina.ca).

Scrapes vet-only products (Pro Plan Veterinary Diets, essentialcare, etc.)
from purina.ca. Retail Purina products are scraped from PetSmart instead
(see purina_retail.py) because purina.ca's Gatsby API stores retail
ingredient data as incomplete taxonomy terms.
    ...
    for heading in soup.find_all(["h3", "h2", "h4"]):
        ...
        if heading_text == "ingredients" or heading_text.startswith("ingredient"):
            ...
            if sibling:
                ...
                if len(ing_text) > 20:
                    ing_text = _PRODUCT_CODE_RE.sub("", ing_text)
                    ing_text = re.sub(r"\boat ber\b", "oat fiber", ing_text)
                    ing_text = re.sub(r"\bnatural avor\b", "natural flavor", ing_text)
                    result["ingredients_raw"] = ing_text
            break
...
def _parse_product(
    ...
    # Ingredients from Gatsby taxonomy terms
...
def scrape_purina_vet(output_dir: Path) -> int:
    """Scrape Purina vet-channel dog products from purina.ca. Returns product count.

    Vet products (Pro Plan Veterinary Diets, essentialcare) have real ingredient
    data on purina.ca — no PetSmart fallback needed. Retail products are handled
    separately by purina_retail.py via PetSmart.
    """
    with SyncSession(rate_limit=1.0) as session:
        ...
        # Pre-filter: skip listings whose URL clearly belongs to a retail brand.
        # This avoids fetching page-data for ~150 retail products just to discard them.
        _RETAIL_URL_PREFIXES = (
            "/pro-plan/", "/purina-one/", "/dog-chow/", "/puppy-chow/",
            "/beneful/", "/beyond/", "/alpo/", "/dentalife/", "/beggin/",
        )
        vet_listings = []
        for listing in listings:
            url_path = (listing.get("url", "") or listing.get("path", "")).lower()
            if any(url_path.startswith(p) for p in _RETAIL_URL_PREFIXES):
                continue
            vet_listings.append(listing)
        logger.info(f"After retail URL filter: {len(vet_listings)} candidates (skipped {len(listings) - len(vet_listings)} obvious retail)")

        products: list[Product] = []
        skipped_retail = 0
        for i, listing in enumerate(vet_listings):
            url_path = listing.get("url", "") or listing.get("path", "")
            if not url_path:
                logger.warning(f"No URL for product: {listing.get('title', '?')}")
                continue

            logger.info(f"  [{i + 1}/{len(vet_listings)}] {listing.get('title', '?')}")
            ...
            # Final channel check via brand taxonomy ID
            if product.get("channel") != "vet":
                skipped_retail += 1
                continue
        ...
        logger.info(f"Vet products: {len(products)} (skipped {skipped_retail} retail)")
        ...
        # Download images (Purina CDN blocks plain httpx)
        _download_purina_images(session, products)

    write_brand_json("Purina", WEBSITE_URL, products, output_dir, slug="purina_vet")
    return len(products)
