"""Iams scraper (via PetSmart.ca).

Data source: petsmart.ca — Next.js with React Server Components (RSC).
- Discovery: listing page at /dog/food/f/brand/iams + XML sitemaps as supplement
...
Key notes:
- Mars Petcare brand. ~23 dog products on PetSmart.ca: dry, wet.
- iams.ca stores all nutritional data as images — PetSmart has it as text.
- Sub-brands: Proactive Health, Advanced Health
...
def _fetch_product_urls(session: SyncSession) -> list[str]:
    """Discover Iams dog product URLs from PetSmart listing page + sitemaps."""
    urls: set[str] = set()
    ...
    logger.info(f"Found {listing_count} Iams dog URLs from listing page")
    ...
    for sitemap_url in _SITEMAP_URLS:
        ...
        if not resp.ok:
            continue
        for m in re.finditer(
            r"<loc>(https://www\.petsmart\.ca/dog/[^<]*iams[^<]*\.html)</loc>",
            resp.text,
            re.IGNORECASE,
        ):
            urls.add(m.group(1))
    ...
    logger.info(
        f"Found {len(urls)} total Iams dog URLs "
...
def _parse_ga(text: str) -> GuaranteedAnalysis | None:
    ...
    - Semicolon-separated single line (Iams wet food)
    ...
    for line in text.split("\n"):
        ...
        if pct_count > 1:
            # Split on comma or semicolon before uppercase letter
...
def _detect_sub_brand(name: str) -> str | None:
    """Detect Iams sub-brand from name."""
    name_lower = name.lower()
    if "advanced health" in name_lower:
        return "Advanced Health"
    if "proactive health" in name_lower:
        return "Proactive Health"
    return None
...
def _parse_product(url: str, html: str) -> Product | None:
    """Parse an Iams product page from PetSmart."""
    ...
    # Skip multi-packs / variety packs — individual products scraped separately
    name_lower = name.lower()
...
def scrape_iams(output_dir: Path) -> int:
    """Scrape all Iams dog food products from PetSmart. Returns product count."""
