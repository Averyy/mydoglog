"""Nutro scraper (via PetSmart.ca).

Data source: petsmart.ca — Next.js with React Server Components (RSC).
- Discovery: listing page at /dog/food/f/brand/nutro + XML sitemaps as supplement
...
Key notes:
- Mars Petcare brand. ~48 dog products on PetSmart.ca: dry, wet.
- nutro.ca stores nutritional data as images — PetSmart has it as text.
- Sub-brands: Natural Choice, NUTRO ULTRA, Limited Ingredient Diet, Wholesome Essentials
...
_LISTING_URL = f"{WEBSITE_URL}/dog/food/f/brand/nutro"
...
def _fetch_product_urls(session: SyncSession) -> list[str]:
    """Discover Nutro dog product URLs from PetSmart listing page + sitemaps."""
    urls: set[str] = set()
    ...
    logger.info(f"Found {listing_count} Nutro dog URLs from listing page")
    ...
    for sitemap_url in _SITEMAP_URLS:
        ...
        if not resp.ok:
            continue
        for m in re.finditer(
            r"<loc>(https://www\.petsmart\.ca/dog/[^<]*nutro[^<]*\.html)</loc>",
            resp.text,
            re.IGNORECASE,
        ):
            urls.add(m.group(1))
    ...
    logger.info(
        f"Found {len(deduped)} total Nutro dog URLs "
...
def _detect_sub_brand(name: str) -> str | None:
    """Detect Nutro sub-brand from name."""
    name_lower = name.lower()
    if "ultra" in name_lower:
        return "NUTRO ULTRA"
    if "limited ingredient" in name_lower:
        return "Limited Ingredient Diet"
    if "wholesome essentials" in name_lower:
        return "Wholesome Essentials"
    if "natural choice" in name_lower:
        return "Natural Choice"
    return None
...
def _parse_product(url: str, html: str) -> Product | None:
    """Parse a Nutro product page from PetSmart."""
    ...
    # Strip "Nutro " or "NUTRO " brand prefix
    name = re.sub(r"^Nutro\s+", "", name, flags=re.IGNORECASE)
...
def _deduplicate_products(products: list[Product]) -> list[Product]:
    ...
    2. Size-variant dupes — different names but same (life_stage, breed_size,
       format, sub_brand, ingredients) → keep the one with the shortest name
       (the canonical listing, not the "Flavour" / reformatted variant).
    """
...
def scrape_nutro(output_dir: Path) -> int:
    """Scrape all Nutro dog food products from PetSmart. Returns product count."""
    ...
    write_brand_json("Nutro", WEBSITE_URL, products, output_dir, slug="nutro")
    return len(products)
